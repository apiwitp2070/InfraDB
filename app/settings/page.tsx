...
export default function TokenSettingsPage() {
  ...
  }, [cloudflareAccountId, cloudflareToken, gitlabBaseUrl, gitlabToken, githubBaseUrl, githubToken])
  ...
  const feedbackColor =
    feedbackType === 'cleared' ? 'warning' : feedbackType === 'error' ? 'danger' : 'success'
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col items-start gap-1">
          ...
          <p className="text-sm text-default-500">
            Tokens persist inside browser IndexedDB. Provide credentials with the required scopes to
            interact with GitLab, GitHub, and Cloudflare APIs.
          </p>
        </div>
      ...
      <section className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-xs text-default-500">Needs repo scope to manage secrets.</p>
        </div>
        <Input
          isDisabled={!isReady}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button
            isDisabled={!isReady || (!tokens.gitlab && !tokens.github && !tokens.cloudflare)}
            variant="light"
            onPress={handleClear}
          >
            Clear
          </Button>
          <Button color="primary" isDisabled={!isReady || !hasChanges} onPress={handleSave}>
            Save tokens
          </Button>
        </div>
      </div>

      <Modal isOpen={isImportExportOpen} onOpenChange={handleImportExportOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">
                {importExportMode === 'import' ? 'Import settings' : 'Export settings'}
              </ModalHeader>
              <ModalBody>
                {importExportMode === 'import' ? (
                  <>
                    <p className="text-sm text-default-500">
                      Paste keys using KEY=VALUE format. Recognized keys: gitlab, github,
                      cloudflare, gitlab_base_url, github_base_url, cloudflare_account_id.
                    </p>
                    <textarea
              ...
              <ModalFooter>
                <Button variant="light" onPress={() => handleCloseImportExport()}>
                  Close
                </Button>
                {importExportMode === 'import' ? (
